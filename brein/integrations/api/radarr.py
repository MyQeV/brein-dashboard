...
async def test_connection(base_url: str, api_key: str) -> tuple[bool, str]:
    """Test connection to Radarr (GET /api/v3/system/status). Returns (success, message)."""
...
async def get_movies(base_url: str, api_key: str) -> tuple[bool, list | None]:
    """GET /api/v3/movie. Returns (True, list of movies) or (False, None)."""
    ok, data = await get_json_with_api_key(base_url, "/api/v3/movie", api_key)
    if not ok or data is None:
        return False, None
    return True, data if isinstance(data, list) else []


async def get_collections(base_url: str, api_key: str) -> tuple[bool, list | None]:
    """GET /api/v3/collection. Returns (True, list of collections) or (False, None)."""
    ok, data = await get_json_with_api_key(base_url, "/api/v3/collection", api_key)
    if not ok or data is None:
        return False, None
    return True, data if isinstance(data, list) else []
...
async def get_movie(
    base_url: str, api_key: str, movie_id: int | str
) -> tuple[bool, dict | None]:
    """GET /api/v3/movie/{id}. Returns (True, data) or (False, None)."""
    return await get_json_with_api_key(base_url, f"/api/v3/movie/{movie_id}", api_key)


# Radarr and Sonarr default to pageSize=10, and the queue view presents what
# it is given as the whole queue — so a 25-item queue silently showed 10. One
# page was still a cap, so the pages are walked until a short one, the
# server's own totalRecords, or QUEUE_MAX_PAGES.
QUEUE_PAGE_SIZE = 200
QUEUE_MAX_PAGES = 20


async def get_queue(base_url: str, api_key: str) -> tuple[bool, dict | None]:
    """GET /api/v3/queue, every page. Returns (True, data) with all records merged or (False, None)."""
    first: dict | None = None
    records: list = []
    for page in range(1, QUEUE_MAX_PAGES + 1):
        ok, data = await get_json_with_api_key(
            base_url,
            f"/api/v3/queue?page={page}&pageSize={QUEUE_PAGE_SIZE}",
            api_key,
        )
...
async def get_history(
    ...
    if event_type and event_type in _RADARR_EVENT_TYPE:
        params["eventType"] = _RADARR_EVENT_TYPE[event_type]
    if start_date:
        params["startDate"] = (
...
async def get_history_movie(
    ...
    """GET /api/v3/history/movie?movieId=... Returns (True, data) or (False, None)."""
    path = "/api/v3/history/movie?" + urlencode({"movieId": movie_id})
    return await get_json_with_api_key(base_url, path, api_key)
...
async def get_calendar(
    ...
    """GET /api/v3/calendar for date range. Returns (True, list of movie objects) or (False, None)."""
