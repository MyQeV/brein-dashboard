...
# it is given as the whole queue — so a 25-item queue silently showed 10.
QUEUE_PAGE_SIZE = 200


async def get_queue(base_url: str, api_key: str) -> tuple[bool, dict | None]:
    """GET /api/v3/queue. Returns (True, data) or (False, None)."""
    return await get_json_with_api_key(
        base_url, f"/api/v3/queue?pageSize={QUEUE_PAGE_SIZE}", api_key
    )
