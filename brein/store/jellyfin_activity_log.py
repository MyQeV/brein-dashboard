"""Store for Jellyfin activity log entries (System/ActivityLog/Entries) per instance."""
...
def _row_dict_from_item(item: dict[str, Any], instance_id: int) -> dict[str, Any]:
    """Build a parameter dict for jellyfin_activity_log_entries from an API item."""
    ...
    user_id = (str(raw_user_id).strip() or None) if raw_user_id is not None else None
    return {
    ...
    async with get_session_factory()() as session:
        for i in range(0, len(entries), _BATCH_SIZE):
            ...
            await session.execute(
                text(
                    "INSERT INTO jellyfin_activity_log_entries"
...
async def get_max_entry_id(instance_id: int) -> int | None:
    ...
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT MAX(entry_id) FROM jellyfin_activity_log_entries WHERE instance_id = :instance_id"
            ),
            {"instance_id": instance_id},
        )
        row = result.fetchone()
...
async def get_min_date_after_entry_id(instance_id: int, entry_id: int) -> str | None:
    ...
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT MIN(date) FROM jellyfin_activity_log_entries"
...
async def get_max_date(instance_id: int) -> str | None:
    ...
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT MAX(date) FROM jellyfin_activity_log_entries WHERE instance_id = :instance_id"
            ),
            {"instance_id": instance_id},
        )
        row = result.fetchone()
...
async def get_entries(
    ...
    """Return activity log entries for an instance, optionally filtered. Ordered by entry_id DESC."""
    query = (
        "SELECT instance_id, entry_id, name, type, item_id, date, user_id, overview"
        " FROM jellyfin_activity_log_entries WHERE instance_id = :instance_id"
    )
...
async def get_distinct_types(instance_id: int) -> list[str]:
    ...
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT DISTINCT type FROM jellyfin_activity_log_entries"
