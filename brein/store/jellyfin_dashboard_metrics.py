"""Dashboard metrics from pre-computed daily snapshot tables (Jellyfin)."""
...
async def rebuild_snapshots(instance_id: int | None = None) -> None:
    ...
    async with _rebuild_lock, get_session_factory()() as session:
        ...
        await session.execute(
            text("DELETE FROM jellyfin_metrics_snapshot" + inst_del_filter),
            inst_del_params,
        )
        await session.execute(
            text(
                f"""
                INSERT INTO jellyfin_metrics_snapshot
        ...
        await session.execute(
            text("DELETE FROM jellyfin_metrics_snapshot_user" + inst_del_filter),
            inst_del_params,
        )
        await session.execute(
            text(
                f"""
                INSERT INTO jellyfin_metrics_snapshot_user
        ...
        await session.execute(
            text("DELETE FROM jellyfin_metrics_snapshot_item" + inst_del_filter),
            inst_del_params,
        )
        await session.execute(
            text(
                f"""
                INSERT INTO jellyfin_metrics_snapshot_item
                ...
                SELECT
                    ...
                    CASE
                        ...
                        THEN series_item.name || ' \u2013 ' || COALESCE(i.name, s.item_id)
                        ELSE COALESCE(i.name, s.item_id)
                    END,
                    COUNT(*) AS plays
                FROM jellyfin_playback_sessions s
                LEFT JOIN jellyfin_items i
    ...
    if user_ids:
        ...
        result = await session.execute(
            text(
                ...
                f" FROM jellyfin_metrics_snapshot_item"
...
async def _get_total_watched_series(
    ...
    if user_ids:
        ...
        result = await session.execute(
            text(
                f"SELECT COUNT(DISTINCT (instance_id, series_id)) FROM jellyfin_metrics_snapshot_item"
...
async def _get_plays_per_user(
    ...
    result = await session.execute(
        text(
            ...
            FROM jellyfin_metrics_snapshot_user
...
async def _get_watch_time_per_user(
    ...
    result = await session.execute(
        text(
            ...
            FROM jellyfin_metrics_snapshot_user s
        ...
        result = await session.execute(
            text(
                ...
                SELECT instance_id, item_id,
                       ...
                       SUM(plays) AS plays
                FROM jellyfin_metrics_snapshot_item
                WHERE 1=1{dr}{inst_w}
...
async def _get_streaming_by_hour(
    ...
    result = await session.execute(
        text(
            f"""
            SELECT CAST(EXTRACT(HOUR FROM (LEFT(start_time, 19)::timestamp AT TIME ZONE 'UTC' AT TIME ZONE :tz)) AS INTEGER) AS hour,
...
async def _get_activity_by_weekday(
    ...
    result = await session.execute(
        text(
            f"""
            SELECT CAST(EXTRACT(DOW FROM (LEFT(start_time, 19)::timestamp AT TIME ZONE 'UTC' AT TIME ZONE :tz)::date) AS INTEGER)
                   AS weekday,
                   COALESCE(SUM(duration_seconds), 0) AS total_seconds
            FROM jellyfin_playback_sessions
            WHERE 1=1
    ...
    async with get_session_factory()() as session:
        ...
        sql = f"""
            ...
            LEFT JOIN jellyfin_users u
               ...
               AND u.user_id = s.user_id
            LEFT JOIN app_instances ai ON ai.id = s.instance_id
            WHERE {_JELLYFIN_MEDIA_TYPE_CASE} = :media_type
              AND (LEFT(s.start_time, 19)::timestamp AT TIME ZONE 'UTC' AT TIME ZONE :tz)::date::text >= :start_date
              AND (LEFT(s.start_time, 19)::timestamp AT TIME ZONE 'UTC' AT TIME ZONE :tz)::date::text <= :end_date
              {_UTC_TEXT_WINDOW}
              {inst_w}{usr_w}
...
async def _get_watch_time_per_user_per_day(
    ...
    The Emby and Plex modules have always produced this; Jellyfin never did,
    and `_merge_metrics` reads it with `.get(...) or []` — so Jellyfin users
    silently never appeared in the Daily chart.
    """
