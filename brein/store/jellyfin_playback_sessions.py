"""Build and store jellyfin_playback_sessions from jellyfin_activity_log_entries."""
...
# Jellyfin logs a start and a stop per media kind (PlaybackStartLogger names
# them Audio*/Video*). Emby's playback.start covers every kind; taking only
# the video pair here left the Audio bucket at zero for Jellyfin forever.
PLAYBACK_START_TYPES = frozenset(("VideoPlayback", "AudioPlayback"))
PLAYBACK_STOP_TYPES = frozenset(("VideoPlaybackStopped", "AudioPlaybackStopped"))
...
async def get_last_processed_entry_id(instance_id: int) -> int | None:
    """Return last_entry_id from jellyfin_playback_sessions_state for this instance, or None."""
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT last_entry_id FROM jellyfin_playback_sessions_state"
...
async def set_last_processed_entry_id(instance_id: int, entry_id: int) -> None:
    """Upsert last_entry_id for this instance into jellyfin_playback_sessions_state."""
    async with get_session_factory()() as session:
        await session.execute(
            text(
                "INSERT INTO jellyfin_playback_sessions_state (instance_id, last_entry_id)"
...
def _build_sessions_from_events(
    ...
    Returns list of dicts for jellyfin_playback_sessions INSERT.
    """
...
async def rebuild_sessions(
    ...
    """Rebuild jellyfin_playback_sessions from activity log using a state-machine.
        ...
        if since_date:
            affected_result = await session.execute(
                text(
                    ...
                    FROM jellyfin_activity_log_entries
            ...
            await session.execute(
                text(
                    "CREATE TEMP TABLE IF NOT EXISTS _jf_rebuild_groups"
                    " (instance_id INTEGER, user_id TEXT, item_id TEXT)"
                )
            )
            await session.execute(text("DELETE FROM _jf_rebuild_groups"))
            await session.execute(
                text(
                    "INSERT INTO _jf_rebuild_groups (instance_id, user_id, item_id)"
                    " VALUES (:iid, :uid, :item)"
                ),
            ...
            await session.execute(
                text(
                    "DELETE FROM jellyfin_playback_sessions s"
                    " USING _jf_rebuild_groups g"
            ...
            fetch_result = await session.execute(
                text(
                    ...
                    FROM jellyfin_activity_log_entries e
                    INNER JOIN _jf_rebuild_groups g
            ...
            await session.execute(text("DROP TABLE IF EXISTS _jf_rebuild_groups"))
        else:
            del_sql = "DELETE FROM jellyfin_playback_sessions"
...
async def reset_all_sessions() -> None:
    """Delete all rows from jellyfin_playback_sessions and jellyfin_playback_sessions_state."""
    async with get_session_factory()() as session:
        await session.execute(text("DELETE FROM jellyfin_playback_sessions"))
        await session.execute(text("DELETE FROM jellyfin_playback_sessions_state"))
        await session.commit()
