"""Jellyfin item metadata store. Bulk-synced from /Items?IncludeItemTypes=... per type."""
...
_UPSERT_IF_CHANGED = (
    " WHERE (jellyfin_items.type, jellyfin_items.name, jellyfin_items.server_id,"
    "  jellyfin_items.series_id, jellyfin_items.season_id, jellyfin_items.parent_id,"
    "  jellyfin_items.run_time_ticks, jellyfin_items.index_number,"
    "  jellyfin_items.parent_index_number)"
    " IS DISTINCT FROM"
...
async def upsert_item(
    ...
    """Insert or replace one row in jellyfin_items."""
    if not item_id:
        return
...
async def upsert_items_bulk(
    ...
    """Bulk upsert a list of Jellyfin item dicts into jellyfin_items. Returns count of rows upserted."""
    if not items:
        return 0
    now = datetime.now(timezone.utc).isoformat()
...
async def get_items(
    ...
    """Return jellyfin_items for an instance, ordered by name ASC."""
    query = (
        "SELECT instance_id, item_id, type, name, server_id, series_id, season_id,"
        " parent_id, run_time_ticks, index_number, parent_index_number, updated_at"
        " FROM jellyfin_items WHERE instance_id = :instance_id"
        " ORDER BY name ASC"
    )
...
async def get_last_scan_date(instance_id: int) -> str | None:
    """Return last_scan_date from jellyfin_items_state for this instance, or None if not set."""
    async with get_session_factory()() as session:
        result = await session.execute(
            text(
                "SELECT last_scan_date FROM jellyfin_items_state WHERE instance_id = :instance_id"
            ),
            {"instance_id": instance_id},
        )
        row = result.fetchone()
...
async def set_last_scan_date(instance_id: int, last_scan_date: str) -> None:
    """Upsert last_scan_date for this instance into jellyfin_items_state."""
    if not last_scan_date:
        return
    async with get_session_factory()() as session:
        await session.execute(
            text(
                "INSERT INTO jellyfin_items_state (instance_id, last_scan_date)"
