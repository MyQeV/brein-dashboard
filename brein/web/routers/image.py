...
ImageAuth = Annotated[User | None, Depends(web_auth.get_current_user_or_image_sig)]
...
async def _proxy_arr_image(instance_id: int, path: str, service_type: str) -> Response:
    """Proxy image from Radarr or Sonarr instance. Path must start with /."""
...
async def api_instance_radarr_image(
    ...
    """Proxy Radarr MediaCover image. Auth via Bearer or signed exp+sig (for <img src>). Responses are cached."""
...
async def api_instance_sonarr_image(
    ...
    """Proxy Sonarr MediaCover image. Auth via Bearer or signed exp+sig (for <img src>). Responses are cached."""
